  ...
  return (
    <header className="w-full border-b bg-white">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        <a href="/" className="text-lg font-semibold">AiBani</a>
              ...
              <div key={item.label} className="relative group text-sm text-slate-700">
                ...
                <ul className="hidden group-hover:block group-focus-within:block absolute top-full left-0 bg-white border rounded-md shadow-sm py-1 min-w-[12rem]">
                  {item.children.map(child => (
                    <li key={child.href}>
                      <a href={child.href} className="block px-3 py-2 hover:bg-slate-50">{child.label}</a>
                    </li>
                  ))}
                </ul>
              </div>
            ) : (
              <a key={item.href} className="text-sm text-slate-700" href={item.href}>{item.label}</a>
            )
          )}
        </nav>
